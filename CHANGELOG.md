...
**視覚ドキュメントの品質下限の引き上げ**。導入案件でMermaid直出しの図が品質不足と判定されたことを受け、図基準の既定を反転した。①人間が見る図は図種を問わずHTML生成ビューが標準（従来はシーケンス図・ER図・状態遷移図をMermaidのまま許容 → 例外を「小さい図」の3条件に限定）②視覚ドキュメントの生成モデル基準を新設（Opus 5以上。軽量モデルのセッションは `model: opus` 指定のサブエージェントへ委譲）。正本=Mermaid/Markdown・一方向生成ビューの原則は変えていない（MINOR）。

### Changed
- **deliverables-catalog「図の正本/ビュー基準」**: 既定を反転。人間承認・顧客レビュー・納品に使う図は図種を問わずHTMLビュー必須。Mermaidレンダリングのままでよいのは「ノード10以下・エッジ交差なし・線形」を**すべて**満たす図のみ（迷ったらHTML）。AI・開発者だけが読む内部作業図はビュー不要のまま
- **basic-design / detail-design / erd-update / business-process-analyst**: 上記の既定反転を各所へ配線（「シーケンス図・ER図・状態遷移図はMermaidのままでよい」の例外を廃止。ERDは `erd.html` の人間承認ビジュアルを標準化）
