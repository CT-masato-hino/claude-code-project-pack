...
外部記事（AIを活用したデザイン業務の構造化事例）から要素を取り込み、UI/UXデザインの判断・レビューを担う専門エージェントを新設。あわせて、案件のデザイン体制（専任デザイナーの有無）に応じてこのエージェントの採用可否を出し分けられるよう、`/project-init` にヒアリング項目と決定表分岐を追加。
...
- **project-init のデザイン体制ヒアリング**（ブロックD-18）: UIを持つ案件で専任デザイナーの有無・所在・不在時の判断代行者と、利用者自身の役割を確認し、ui-ux-designerの採用可否をフェーズ3決定表で判定できるようにした
...
- ui-ux-designerをflow-map・tailoring-guide・ipa-process-mapping・deliverables-catalog・feature-team.md（複製禁止リスト）に反映
