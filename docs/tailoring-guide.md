...
この軸は「1. 案件規模別の推奨構成」と直交する。小規模案件でも依頼者の専門外ドメインがあれば該当エージェントの原則運用を厳格化し、大規模案件でも依頼者・チームが全ドメインの専門家なら人間レビューを主軸にできる。判定結果は CLAUDE.md の依頼者ロール節と、フェーズ3決定表の「体制ギャップ」行に記録する。
