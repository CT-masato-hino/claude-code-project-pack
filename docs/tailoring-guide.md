...
- **停止基準を必ず明文化する（納期・顧客のない内製・ホビー用途では特に）**: 本パックの品質機構（Q-01 第三者レビュー100% × 指摘を暗黙に握りつぶさない運用）は、外部の納期・検収という自然なブレーキがない案件では「レビューが指摘を生み→指摘がissueを生み→issueが再検証を生む」自己増殖ループに転化しうる。「Mustブロッカー0件なら次工程に進んでよい。残Should/Nitsは工程完了の条件にしない」をデフォルトの停止基準としてCLAUDE.mdのQCD節に確定値で書く（leaderの「品質ループの停止判断」チェックリストと連動）
