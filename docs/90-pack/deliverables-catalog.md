...
- **人間確認は週次進捗・EVの算入条件でもある**（qcd-standards D-01/EVM: 「完了」＝台帳「確認済み」のみ。AI構築済み・未承認は仕掛品0%）。よって人間確認は工程末にまとめず、成果物単位で随時流す — 承認の滞留はそのままSPIの低下として数字に映る
...
- 閲覧は `tools/docs_viewer.py`（Markdown内Mermaid・`.mmd`・HTMLモックをそのまま描画できる）。起動は `/docs-viewer` スキル（コマンドを覚えなくてよい）
...
## 図の正本/ビュー基準（人間が見る図はHTML。Mermaidレンダリングは基本使わない）

Mermaidは正本（git差分・機械可読）としては優秀だが、**人間に見せるビジュアルとしての品質下限を保証できない**（自動レイアウトは「読み順」を作れない。導入案件でMermaid直出しの図が品質不足と判定された実績あり）。よって**人間（顧客・承認者）が見るビジュアルにMermaidレンダリングは基本使わない**:

| 内容の性質 | 正本 | 人間に見せるビジュアル |
|---|---|---|
| 人間承認・顧客レビュー・納品に使う図（業務フロー図・DFD・機能関連図・構成図・シーケンス図・ER図・状態遷移図・画面遷移図・スケジュール — 図種を問わない） | Mermaid / Markdown | **HTML生成ビュー（必須）。Mermaidレンダリングは基本使わない** |
| 表・箇条書きで足りる内容 | Markdown | Markdownのまま（無理に図・HTML化しない） |
| AI・開発者だけが読む内部作業図（人間レビューの対象でない） | Mermaid / Markdown | ビュー不要 |

- **図の品質基準（最重要 — 図は装飾ではなく判断材料）**: 図は次の3問に答えるために作る。①**全体のうち何がどこまで終わっているか**（進捗・現在地）②**どこが難所か・ボトルネック/クリティカルパスはどこか** ③**何が何に依存し、マイルストーンはどこか**。3問のどれにも答えない図・意味のないカードやチップの羅列・SaaSツール風の装飾（アバター・炎アイコン・過剰な色数）は作り直し対象。ゴールデンサンプル: [`master-schedule-template.html`](../../.claude/skills/project-init/templates/master-schedule-template.html)（週次タイムライン・依存矢印・▼マイルストーン・クリティカルパス強調・現在日ライン・出来高。教科書のマスタスケジュール流の手組みレイアウトが正解のレベル）
- **生成モデル基準（視覚ドキュメント共通）**: 図のHTMLビュー・HTMLモック・スライド・成果物一覧HTML等、人間に見せる視覚ドキュメントの新規生成・大幅改稿は **Opus 5 以上のモデル**で行う。セッションが Sonnet/Haiku 等の軽量モデルで走っている場合はその場で生成せず、`model: opus` を明示指定したサブエージェントに委譲する（エージェント定義の model は inherit のまま — 委譲時の指定で担保する）。軽微な文言修正はこの限りではない。軽量モデル・Mermaid自動レイアウト任せの図は品質下限を守れないことが導入案件で実証済み
- **HTMLビューの生成規約**: 正本（Mermaid/Markdown）からの一方向生成・直接編集禁止・再生成可能。デザインは project-plan.html の言語（モノクロSVG・**絵文字禁止**。チップ・カードは状態・分類・数値など情報を運ぶ場合のみ — 装飾としての羅列は禁止）を踏襲。置き場所は正本と同ディレクトリ
