...
## 図の正本/ビュー基準（HTMLビューが標準）

Mermaidは正本（git差分・機械可読）としては優秀だが、**人間に見せるビジュアルとしての品質下限を保証できない**（自動レイアウトは「読み順」を作れない。導入案件でMermaid直出しの図が品質不足と判定された実績あり）。よって**人間（顧客・承認者）が見る図は、図種を問わずHTML生成ビューを標準とする**。Mermaidレンダリングのままでよいのは「Markdownで十分」と言い切れる小さな図だけに限定する:

| 図の用途 | 正本 | 人間に見せるビジュアル |
|---|---|---|
| 人間承認・顧客レビュー・納品に使う図（業務フロー図・DFD・機能関連図・構成図・シーケンス図・ER図・状態遷移図・画面遷移図 — 図種を問わない） | Mermaid / Markdown | **HTML生成ビュー（標準・必須）** |
| 上記のうち ①ノード10以下 ②エッジ交差なし ③構造が線形（一列・一方向）を**すべて**満たす図 | Mermaid | Mermaidレンダリングのままでよい（迷ったらHTML） |
...
- **機械的な閾値**: 例外3条件（ノード10以下・交差なし・線形）を1つでも外れたらHTMLビューを作る（判断を人に委ねない）
